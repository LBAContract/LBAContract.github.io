...
contract TRC21 is ITRC21 {
    ...
    function transferFrom(address from,	address to,	uint256 value) public override returns (bool) {
        uint256 total = value.add(_minFee);
        require(to != address(0),"Transfer To not be null");
        require(total <= _allowed[from][msg.sender],"Not Enough Money");
    ...
    function _transfer(address from, address to, uint256 value) internal {
        require(value <= _balances[from], "Not Enough Money");
        require(to != address(0));
...
contract MyTRC21Mintable is TRC21 {
    string private _name;
    string private _symbol;
    uint8 private _decimals;

    constructor (string memory name, string memory symbol, uint8 decimals, uint256 cap, uint256 minFee) {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
